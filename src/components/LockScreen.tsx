...
interface CheckLockInterface {
    ...
    customBottomComponent?: (currentInput: string[], setCurrentInput: Dispatch<string[]>, wrongAttempts: number, blockedInput: boolean) => void
...
const LockScreen: React.FC<LockScreenProps> = React.memo((props) => {
    ...
    const [blockedInput, setBlockedInput] = React.useState<boolean>(!!props.initBlockTime);
    const [blockedTime, setBlockedTime] = React.useState<number>(props.initBlockTime ? props.initBlockTime : 0);
    const [blockedErrorMsg, setBlockedErrorMsg] = React.useState<string>(props.initErrorMsg ? props.initErrorMsg : '');
    ...
    React.useEffect(() => {
        ...
        if (wrongAttempts in props.errors) {
            ...
            setBlockedErrorMsg(errorObj.errorMessage.replace('#_BLOCKED_TIME_COUNTDOWN_#', getTime(blockedTime)))
        }
    }, [blockedTime])
    ...
    React.useEffect(() => {
        ...
        if (props.validInput.length === currentInput.length) {
            let equals = props.validInput.every((val, index) => val === currentInput[index]);
            if (equals) {
                setCurrentInput([])
    ...
    const renderTop = React.useMemo(() => {
        ...
        return (
            <LockScreenTop
                label={props.buildInTopComponentOptions?.label ? props.buildInTopComponentOptions?.label : ''}
                validInput={props.validInput}
                currentInput={currentInput}
                wrongAttempts={wrongAttempts}
                errorMsg={blockedErrorMsg}
                identifierColors={{
                    filled: props.buildInTopComponentOptions?.filledColor ? props.buildInTopComponentOptions.filledColor : '#00A1FC',
                    unfilled: props.buildInTopComponentOptions?.unfilledColor ? props.buildInTopComponentOptions.unfilledColor : '#696767'
                }}
            />
        )
    ...
    const renderBottom = React.useMemo(() => {
        if (typeof props.customBottomComponent !== 'undefined') {
            return props.customBottomComponent(props.input, setCurrentInput, wrongAttempts, blockedInput)
        }
        ...
        return (
            <LockScreenBottom
                ...
                bottomComponentLabel={props.buildInBottomComponentOptions ? props.buildInBottomComponentOptions.bottomComponent.label : ''}
                bottomComponentOnPress={props.buildInBottomComponentOptions ? props.buildInBottomComponentOptions.bottomComponent.onPress : () => {}}
            />
        );
